...
_BILLING_STATES.update({
        'required': (Eval('pysriws_concept') == '2')
                    | (Eval('pysriws_concept') == '3')
    })
...
IVA_SRI_CODE = collections.defaultdict(lambda: 0)
IVA_SRI_CODE.update({
    Decimal('0'): 3,
    Decimal('0.105'): 4,
    Decimal('0.21'): 5,
...
class Invoice:
    ...
    def __setup__(cls):
        ...
        cls._error_messages.update({
            ...
            'invalid_invoice_number':
                ...
                u'la SRI (%d). Modifique la secuencia del diario',
            'not_cae':
    ...
    def on_change_pos(self):
        ...
        if company_iva == 'responsable_inscripto':
            ...
            elif self.party.vat_country == u'EC':
                kind = 'B'
            else:
                kind = 'E'
        else:
        ...
        for invoice in invoices:
            ...
            if invoice.pos:
                if invoice.pos.pos_type == 'electronic':
                    invoice.do_pysriws_request_cae()
    ...
    def do_pysriws_request_cae(self):
        logger = logging.getLogger('pysriws')
        "Request to SRI the invoices' Authorization Electronic Code (CAE)"
        # if already authorized (electronic invoice with CAE), ignore
        if self.pysriws_cae:
            logger.info(u'Se trata de obtener CAE de la factura que ya tiene. '\
                        u'Factura: %s, CAE: %s', self.number, self.pysriws_cae)
            return
        ...
        # authenticate against SRI:
        auth_data = company.pysriws_authenticate(service=service)

        # import the SRI webservice helper for electronic invoice
        if service == 'wsfe':
        ...
        ws.Conectar()
        # set SRI webservice credentials:
        ...
        # get the last invoice number registered in SRI
        ...
        # verify that the invoice is the next one to be registered in SRI
        ...
        if self.invoice_address.country:
            # map ISO country code to SRI destination country code:
            pais_dst_cmp = {
        ...
        # Request the authorization! (call the SRI webservice method)
        try:
            if service == 'wsfe':
                ws.CAESolicitar()
            elif service == 'wsmtxca':
        ...
        SRI_Transaction = pool.get('account_invoice_ec.sri_transaction')
        with Transaction().new_cursor():
            SRI_Transaction.create([{'invoice': self,
    ...
    def pysriws_verification_digit_modulo10(self, codigo):
        "Calculate the verification digit 'modulo 10'"
        # http://www.consejo.org.ar/Bib_elect/diciembre04_CT/documentos/rsri1702.htm
